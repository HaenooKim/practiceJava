...
class XCanvas extends Canvas {
	...
	/*
		처음 그리고 크기변경시
		App이 캔버스가 다시 그려질 필요가 있다고 판단되는 시점에서 호출된다.
	*/
	public void paint(Graphics g) { //앱이 알아서 호출
	...
	/*
	사용자에 의해서 그림이 다시 그려질 필요가 있을 때 호출되는 함수.
	바둑판그림에서 > 버툰누르면 > repaint호출> updatei > 바둑판 그리고 >점찍는다.
	
	버튼을 누르거나 클릭이 일어났을때 그림이 다시 그려져야 할 필요가 있을때는
	update에서 그림을 그리게 만들고 paint에서는 update를 호출하는 형태로 오버라이딩한다.
	*/
...
class XFrame extends Frame {
	public XFrame() {
		...
		/*
			...
			GUI 관련된 이벤트처리 , 즉 XXX 상황이 벌어졌을떄 동작하는 코드에 대해서는
